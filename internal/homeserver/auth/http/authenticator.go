...
// Returns UnauthenticatedError if user is not be authenticated.
func (a *SessionAuthenticator) Authenticate(r *http.Request) (AuthenticateResult, error) {
	...
	if errors.Is(err, http.ErrNoCookie) {
		return AuthenticateResult{}, fmt.Errorf("no access token cookie: %w", UnauthenticatedError)
	}
	if err != nil {
	...
	if !found {
		return AuthenticateResult{}, fmt.Errorf("access token not found: %w", UnauthenticatedError)
	}
